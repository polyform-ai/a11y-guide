...
export function renderAgentReadyReport(options: AgentReadyReportOptions): string {
  ...
  const groupedIssueCount = pageGroups.reduce((total, groups) => total + groups.length, 0)
  ...
  const pageSections = options.pages.map((page, pageIndex) => {
    const groups = pageGroups[pageIndex]!
    return `
    <section class="page" id="page-${pageIndex + 1}">
      ...
      ${groups.length ? `<div class="findings">${groups.map((group) => `<article class="finding"><div><span class="impact ${group.impact}">${escapeHtml(group.impact)}</span><code>${escapeHtml(group.rule)}</code>${group.count > 1 ? `<span class="count">${group.count} occurrences</span>` : ''}</div><h4>${escapeHtml(group.message)}</h4><p>${escapeHtml(group.recommendation)}</p>${group.selectors.length === 1 ? `<code class="selector">${escapeHtml(group.selectors[0])}</code>` : group.selectors.length > 1 ? `<details><summary>View ${group.selectors.length} affected selectors</summary><div class="selectors">${group.selectors.map((selector) => `<code>${escapeHtml(selector)}</code>`).join('')}</div></details>` : ''}</article>`).join('')}</div>` : '<p class="empty">No findings from the automated checks on this rendered state.</p>'}
    </section>`
  }).join('')
